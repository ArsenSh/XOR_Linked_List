template <typename T>
typename List<T>::Node* List<T>::XOR(Node* x, Node *y)
{
...
{
    copy -> size_of_list = size_of_list;
    Node * itrsave;
...
//    using first = class List<T>::first;
//    if(first == nullptr){COUT << "nullptr\n"; return COUT;}
//    Node* itrsave;
//    Node* lastitr = nullptr;
//    Node* itr = first;
//    while(itr)
...
void List<T>::insert(T& value, std::size_t position)
{
    ...
    T arr[size_of_list - position];
    for(int i = static_cast<int>(position); i < size_of_list; ++i)
    {
        arr[i] = (current -> data);
        current = current -> npx;
    }
    ...
    for(int i = static_cast<int>(size_of_list - 1 - position); i >= 0; --i)
    {
        pop_back();
    }
    push_back(value);
...
//    inserted -> npx = XOR(current, current -> npx);
//    current -> npx = XOR(inserted -> npx, current);
    ++size_of_list;
}
